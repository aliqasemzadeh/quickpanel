    ...
    <div class="relative w-full max-w-4xl p-6 bg-white rounded-lg shadow dark:bg-gray-800">
        <div class="flex items-start justify-between pb-3 border-b dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ __('jetadmin.permissions') }}: {{ isset($user) ? $user->name : '' }}
            </h3>
            <button type="button" class="text-gray-400 hover:text-gray-900 dark:hover:text-white" aria-label="Close" wire:click="close">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
        </div>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">{{ __('jetadmin.permissions_description') }}</p>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('jetadmin.search') }}</label>
                    ...
                    @foreach($permissions as $permission)
                        <li class="py-3">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 dark:focus:ring-green-800" wire:click="assign({{ $permission->id }})" onclick="return confirm('{{ __('jetadmin.are_you_sure') }}')">
                    ...
                    @foreach(isset($user) && isset($user->permissions) ? $user->permissions : [] as $permission)
                        <li class="py-3">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 dark:focus:ring-red-800" wire:click="delete({{ $permission->id }})" onclick="return confirm('{{ __('jetadmin.are_you_sure') }}')">
